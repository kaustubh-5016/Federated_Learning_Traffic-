...
if __name__ == '__main__':
    ...
    client5_space = space_path('client5_space')
    ...
    test_x = test_x[-100:]
    test_y = test_y[-100:]
    model_path = os.path.join(client5_space, 'client5_model.h5')
    weight_path = os.path.join(client5_space, filename)
    if os.path.exists(model_path) and os.path.exists(weight_path):
        ...
        cm.save_weights(os.path.join(client5_space, 'client5_weights.h5'))
        cm.save(model_path)
        ...
        m.save_weights(os.path.join(client5_space, 'client5_weights.h5'))
        m.save(model_path)
        update_and_save_the_learning_curve(client5_space, history, 'loss', 'WASHng')
